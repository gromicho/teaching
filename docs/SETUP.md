...
| ABW linear-optimization exercises and the repeated-solves lesson | Use installed Pyomo/HiGHS; install either package only if missing. |
...
| Alice nonlinear optimization | Uses installed Pyomo, installing it only if absent. SciPy performs the numerical calculation, with SymPy, NumPy and Matplotlib. HiGHS is not a nonlinear solver. |
...
The former exact-version Pyomo/HiGHS installation line was unnecessary when Colab already supplied these packages. It has been replaced by a short dependency check: when all requested packages are present, **pip is not called at all**. When a package is absent, only that package is requested, without an exact version or upgrade flag. Pip may still install dependencies required by that missing package.
